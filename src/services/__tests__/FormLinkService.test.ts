...
import FormLinkService, { InviteLinkData } from "../FormLinkService";
...
export interface ExpirationOptions {
  ...
  hours?: number;
  minutes?: number;
  minute?: number; // Alias for minutes
  seconds?: number;
  second?: number; // Alias for seconds
  milliseconds?: number;
...
describe("FormLinkService", () => {
  ...
  describe("generateInviteLink & validateInviteLink", () => {
    test("generates and validates an active invite link with future expiration", () => {
      ...
      const inviteLink = formLinkService.generateInviteLink(
        payload,
        "/invite",
        2,
      );
    ...
    test("correctly rejects expired invite link created with past expiration", () => {
      ...
      const encrypted = (formLinkService as any).encrypt(
  ...
  describe("Form Link Utilities", () => {
    ...
    test("extractFormIdFromUrl returns null for invalid URL without form-access path", () => {
      ...
      expect(
        formLinkService.extractFormIdFromUrl(`${baseUrl}/other-path/123`),
      ).toBeNull();
      expect(
        formLinkService.extractFormIdFromUrl("invalid-url-string"),
      ).toBeNull();
    ...
    test("generateSecureFormLink generates link with access token and expiration", () => {
      const link = formLinkService.generateSecureFormLink(
        formId.toString(),
        12,
      );

      expect(link.url).toContain(
        `${baseUrl}/form-access/${formId.toString()}/`,
      );
    ...
    test("validateAccessToken accepts 64-character hex tokens and rejects invalid", async () => {
      ...
      expect(
        await formLinkService.validateAccessToken(
          formId.toString(),
          validToken,
        ),
      ).toBe(true);
      ...
      expect(
        await formLinkService.validateAccessToken(formId.toString(), ""),
      ).toBe(false);
    });

    test("generateBatchLinks generates requested quantity of links", () => {
      const links = formLinkService.generateBatchLinks(
        formId.toString(),
        5,
        true,
      );
