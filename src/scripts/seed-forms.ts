...
export async function seedFormData(): Promise<SeededData> {
  try {
    ...
    const [owner, editor, collaborator, respondent1, respondent2, admin] =
      users;
    ...
    const forms = await Form.insertMany([
      {
        ...
        submittype: SubmitType.Multiple,
        setting: {
          qcolor: "#4F46E5",
          bg: "#FFFFFF",
          navbar: "#1E40AF",
    ...
    const surveyContents = await Content.insertMany([
      {
    ...
    const quizContents = await Content.insertMany([
      {
        formId: quizForm._id,
        qIdx: 0,
        questionId: "Q1",
    ...
    const feedbackContents = await Content.insertMany([
      {
    ...
    await Form.findByIdAndUpdate(surveyForm._id, {
      contentIds: surveyContents.map((c) => c._id),
    });
    await Form.findByIdAndUpdate(quizForm._id, {
      contentIds: quizContents.map((c) => c._id),
    });
    await Form.findByIdAndUpdate(feedbackForm._id, {
    ...
    // Survey Responses
    const surveyResponses = await FormResponse.insertMany([
      {
        formId: surveyForm._id,
        ...
        responseset: [
          {
            ...
            scoringMethod: ScoringMethod.NONE,
          },
          {
    ...
    // Quiz Responses
    const quizResponses = await FormResponse.insertMany([
      {
        formId: quizForm._id,
        userId: respondent1._id,
        responseset: [
      ...
      },
      {
        ...
        responseset: [
          ...
          },
          {
            ...
            score: 7,
            comment:
              "Partial credit: 'Character' is not a JavaScript data type",
            scoringMethod: ScoringMethod.MANUAL,
          },
          {
            question: quizContents[2]._id,
    ...
    // Feedback Responses
    const feedbackResponses = await FormResponse.insertMany([
      {
        formId: feedbackForm._id,
        userId: respondent1._id,
        responseset: [
