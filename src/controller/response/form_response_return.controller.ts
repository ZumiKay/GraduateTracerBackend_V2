...
const renderResponseValue = (
  ...
  if (
    ...
    return renderRangeResponse(response, question.type === QuestionType.RangeDate);
  }
  ...
  const options = question[question.type as keyof ContentType] as Array<ChoiceQuestionType>;
      ...
      .map((resp) => {
        ...
        return `
          <div class="question-item" style="background-color: #f9fafb; border: 1px solid #e5e7eb; border-radius: 8px; padding: 20px; margin-bottom: 15px;">
            ...
            ${!isTextType && question.hasAnswer && question.answer && form.type === TypeForm.Quiz ? `
              <div style="background-color: #d1fae5; border-left: 3px solid #10b981; padding: 12px; margin-top: 10px; border-radius: 4px;">
                ...
                <div style="color: #065f46; font-size: 14px;">${renderResponseValue(
                  typeof question.answer === "object" && "answer" in question.answer
...
class FormResponseReturnController extends FormResponseController {
  public ReturnResponse = async (req: CustomRequest, res: Response) => {
    ...
    try {
      const formResponses = await FormResponse.find({ _id: { $in: responseIds } })
      ...
      const numberedContent = AddQuestionNumbering({ questions: contents });

      await Promise.all(
        formResponses.map(async (response) => {
          if (!response.respondentEmail) return;

          let questionsHtml = "";
          if (includeQuestionsAndResponses) {
            const processedResponseSet = ResponseQueryService.ResponsesetProcessQuestion(
              numberedContent,
