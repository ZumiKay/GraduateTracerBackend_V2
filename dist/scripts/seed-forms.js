...
async function seedFormData() {
    try {
        ...
        // 2. Create Forms
        const forms = await Form_model_1.default.insertMany([
            {
        ...
        const surveyContents = await Content_model_1.default.insertMany([
            {
        ...
        const quizContents = await Content_model_1.default.insertMany([
            {
                formId: quizForm._id,
                qIdx: 0,
                questionId: "Q1",
        ...
        const feedbackContents = await Content_model_1.default.insertMany([
            {
        ...
        await Form_model_1.default.findByIdAndUpdate(surveyForm._id, {
            contentIds: surveyContents.map((c) => c._id),
        });
        await Form_model_1.default.findByIdAndUpdate(quizForm._id, {
            contentIds: quizContents.map((c) => c._id),
        });
        await Form_model_1.default.findByIdAndUpdate(feedbackForm._id, {
            contentIds: feedbackContents.map((c) => c._id),
        });
        // 4. Create Form Responses
        const responses = [];
        // Survey Responses
        const surveyResponses = await Response_model_1.default.insertMany([
            {
                formId: surveyForm._id,
        ...
        // Quiz Responses
        const quizResponses = await Response_model_1.default.insertMany([
            {
                formId: quizForm._id,
                userId: respondent1._id,
                responseset: [
        ...
        // Feedback Responses
        const feedbackResponses = await Response_model_1.default.insertMany([
            {
                formId: feedbackForm._id,
                userId: respondent1._id,
                responseset: [
