...
class ExampleUsageController {
     ...
     */
    static async submitFormResponse(req, res) {
        try {
            const formId = req.params.formId;
            // Use the convenient helper method to check from request
            const duplicateCheck = await ResponseValidationService_1.ResponseValidationService.hasRespondentFromRequest(formId, req, {
     ...
     */
    static async checkRespondentStatus(req, res) {
        try {
            const { formId } = req.params;
            const { fingerprint, ipAddress, userId, guestEmail } = req.body;
            // Direct usage with specific options
            const result = await ResponseValidationService_1.ResponseValidationService.hasRespondent(formId, {
                fingerprint,
                ipAddress,
                userId,
                guestEmail,
     ...
     */
    static async checkAuthenticatedUserResponse(req, res) {
        try {
            const { formId } = req.params;
     ...
     */
    static async checkGuestUserResponse(req, res) {
        try {
            const { formId } = req.params;
            ...
            const result = await ResponseValidationService_1.ResponseValidationService.hasRespondent(formId, trackingOptions);
            res.json({
                hasResponded: result.hasResponded,
                trackingMethod: result.trackingMethod,
                confidence: result.confidence,
