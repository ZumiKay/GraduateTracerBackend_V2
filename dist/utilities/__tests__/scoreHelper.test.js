...
const scoreHelper_1 = require("../scoreHelper");
describe("getFormScoringAnalysis", () => {
    ...
    describe("Basic Question Sets", () => {
        test("returns all zeros and isAutoScoreable: false for empty question list", () => {
            const result = (0, scoreHelper_1.getFormScoringAnalysis)({ questions: [] });
        ...
        test("isAutoScoreable should be false if there is a manual grading question", () => {
            const questions = [
                mockdata_1.MockContentFactory.createMultipleChoiceContent({ score: 20 }),
                mockdata_1.MockContentFactory.createNumberContent({ score: 10 }),
        ...
        test("isAutoScoreable should be true when all scored questions have answer keys", () => {
            const questions = [
                ...
                mockdata_1.MockContentFactory.createCheckboxContent({ score: 15 }),
                mockdata_1.MockContentFactory.createNumberContent({ score: 5 }),
                mockdata_1.MockContentFactory.createShortAnswerContent({
                    score: 20,
                    answer: { answer: "answer key", isCorrect: true },
                }),
            ];
            const result = (0, scoreHelper_1.getFormScoringAnalysis)({ questions });
        ...
        test("display-only text questions should never count towards scored or manual grading", () => {
            const questions = [
                mockdata_1.MockContentFactory.createTextContent({ score: 0 }),
                mockdata_1.MockContentFactory.createTextContent({ score: 10 }), // Text questions ignored even if score present
                mockdata_1.MockContentFactory.createMultipleChoiceContent({ score: 10 }),
            ];
            const result = (0, scoreHelper_1.getFormScoringAnalysis)({ questions });
    ...
    describe("Large Question Sets", () => {
        test("accurately counts 500 fully auto-scorable questions", () => {
            ...
            const questions = Array.from({ length: COUNT }, (_, i) => {
                const types = [
                    ...
                    mockdata_1.MockContentFactory.createNumberContent({ formId, qIdx: i, score: 5 }),
                    mockdata_1.MockContentFactory.createDateContent({ formId, qIdx: i, score: 5 }),
                ];
        ...
        test("accurately analyzes 1,000 mixed questions across all categories", () => {
            ...
            const TOTAL = AUTO_COUNT + MANUAL_COUNT + NON_SCORED_COUNT + DISPLAY_TEXT_COUNT; // 1,000
            const questions = [];
            // 1. Auto-scorable (score > 0 with answer keys)
            for (let i = 0; i < AUTO_COUNT; i++) {
                questions.push(mockdata_1.MockContentFactory.createMultipleChoiceContent({
                    formId,
            ...
            for (let i = 0; i < DISPLAY_TEXT_COUNT; i++) {
                questions.push(mockdata_1.MockContentFactory.createTextContent({
                    formId,
                    qIdx: questions.length,
                    score: 0,
                }));
        ...
        test("detects a single manual grading question among 500 auto-scorable questions (boundary test)", () => {
            const TOTAL = 500;
            const questions = Array.from({ length: TOTAL - 1 }, (_, i) => mockdata_1.MockContentFactory.createMultipleChoiceContent({
                formId,
                qIdx: i,
                score: 10,
            }));
            // Insert 1 manual grading question at the end
            questions.push(mockdata_1.MockContentFactory.createParagraphContent({
                formId,
                qIdx: TOTAL - 1,
                score: 25,
                answer: undefined,
        ...
        test("correctly handles 300 non-scored questions", () => {
            const COUNT = 300;
            const questions = Array.from({ length: COUNT }, (_, i) => mockdata_1.MockContentFactory.createMultipleChoiceContent({
                formId,
                qIdx: i,
                score: 0,
            }));
        ...
        test("analyzes 250 ShortAnswer/Paragraph questions with valid answer keys as auto-scorable", () => {
            const COUNT = 250;
            const questions = Array.from({ length: COUNT }, (_, i) => i % 2 === 0
                ? mockdata_1.MockContentFactory.createShortAnswerContent({
                    formId,
                    qIdx: i,
                    score: 10,
                    answer: { answer: `sample answer ${i}`, isCorrect: true },
                })
                : mockdata_1.MockContentFactory.createParagraphContent({
                    formId,
                    qIdx: i,
