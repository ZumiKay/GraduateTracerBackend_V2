...
describe("ResponseValidationService", () => {
    let mockReq;
    let mockRes;
    const validObjectId = new mongoose_1.Types.ObjectId().toString();
    ...
    describe("validateRequest", () => {
        describe("Authentication / Token checks", () => {
            test("returns 404 and isValid=false when user is missing and noToken is not set", () => {
                ...
                const result = ResponseValidationService_1.ResponseValidationService.validateRequest({
                    req: mockReq,
                    res: mockRes,
                });
                expect(mockRes.status).toHaveBeenCalledWith(404);
                expect(mockRes.json).toHaveBeenCalledWith((0, helper_1.ReturnCode)(404));
        ...
        describe("requireFormId = true (default)", () => {
            test("returns 400 when formId is missing", () => {
                ...
                expect(mockRes.json).toHaveBeenCalledWith((0, helper_1.ReturnCode)(400));
                expect(result).toStrictEqual({ isValid: false });
            });
    ...
    describe("validateFormAccess", () => {
        test("returns 404 when form is not found", async () => {
            ...
            const result = await ResponseValidationService_1.ResponseValidationService.validateFormAccess(validObjectId, validUserId, mockRes);
            expect(Form_model_1.default.findById).toHaveBeenCalledWith(expect.any(mongoose_1.Types.ObjectId));
            expect(mockRes.status).toHaveBeenCalledWith(404);
            expect(mockRes.json).toHaveBeenCalledWith((0, helper_1.ReturnCode)(404, "Form not found"));
        ...
        test("returns 403 when user does not have access to the form", async () => {
            ...
            formHelpers_1.hasFormAccess.mockReturnValue(false);
            const result = await ResponseValidationService_1.ResponseValidationService.validateFormAccess(validObjectId, validUserId, mockRes);
            expect(formHelpers_1.hasFormAccess).toHaveBeenCalledWith(mockForm, expect.any(mongoose_1.Types.ObjectId));
            expect(mockRes.status).toHaveBeenCalledWith(403);
            expect(mockRes.json).toHaveBeenCalledWith((0, helper_1.ReturnCode)(403, "Access denied"));
            expect(result).toBeNull();
        });
        test("returns form when access is granted", async () => {
            ...
            formHelpers_1.hasFormAccess.mockReturnValue(true);
            const result = await ResponseValidationService_1.ResponseValidationService.validateFormAccess(validObjectId, validUserId, mockRes);
        ...
        test("returns 500 when an unexpected database error occurs", async () => {
            ...
            expect(mockRes.json).toHaveBeenCalledWith((0, helper_1.ReturnCode)(500, "Failed to validate form access"));
            expect(result).toBeNull();
    ...
    describe("validateResponseAccess", () => {
        ...
        test("returns 404 when form response is not found", async () => {
            ...
            const result = await ResponseValidationService_1.ResponseValidationService.validateResponseAccess(responseId, validUserId, mockRes);
            expect(Response_model_1.default.findById).toHaveBeenCalledWith(responseId);
            expect(mockRes.status).toHaveBeenCalledWith(404);
            expect(mockRes.json).toHaveBeenCalledWith((0, helper_1.ReturnCode)(404, "Response not found"));
            expect(result).toBeNull();
        });
        test("returns 404 when associated form is not found", async () => {
            ...
            const result = await ResponseValidationService_1.ResponseValidationService.validateResponseAccess(responseId, validUserId, mockRes);
            expect(Form_model_1.default.findById).toHaveBeenCalledWith(validObjectId);
            expect(mockRes.status).toHaveBeenCalledWith(404);
            expect(mockRes.json).toHaveBeenCalledWith((0, helper_1.ReturnCode)(404, "Form not found"));
            expect(result).toBeNull();
        });
        ...
        test("returns 500 when an unexpected database error occurs", async () => {
            ...
            const result = await ResponseValidationService_1.ResponseValidationService.validateResponseAccess(responseId, validUserId, mockRes);
            expect(mockRes.status).toHaveBeenCalledWith(500);
            expect(mockRes.json).toHaveBeenCalledWith((0, helper_1.ReturnCode)(500, "Failed to validate response access"));
            expect(result).toBeNull();
            consoleSpy.mockRestore();
        });
    });
