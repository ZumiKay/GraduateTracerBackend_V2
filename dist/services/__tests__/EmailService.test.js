...
describe("EmailService", () => {
    ...
    describe("sendFormLinks", () => {
        test("generates correct HTML and sends form links with string title and message", async () => {
            ...
            expect(mailOptions.html).toContain("https://graduatetracer.example.com/form-access/form-12345");
            expect(mailOptions.html).toContain("Graduate Survey 2026");
            expect(mailOptions.html).toContain("University Admin");
            expect(mailOptions.html).toContain("Please complete this survey by next week.");
        });
        test("handles ContentTitle object and escapes HTML characters in formTitle and formOwner", async () => {
            ...
            expect(result).toBe(true);
            const mailOptions = mockSendMail.mock.calls[0][0];
            expect(mailOptions.subject).toBe("Form Invitation: Survey <Alpha> & 'Beta' \"Gamma\"");
            // In HTML body, it should be escaped
            expect(mailOptions.html).toContain("Survey &lt;Alpha&gt; &amp; &#039;Beta&#039; &quot;Gamma&quot;");
    ...
    describe("sendResponseCardEmail", () => {
        ...
        test.each([
        ...
        ])("assigns correct grade '$grade' and emoji '$emoji' for score percentage $percentage%", async ({ percentage, grade, emoji, color, bgColor }) => {
            const data = {
            ...
            expect(result).toBe(true);
            const mailOptions = mockSendMail.mock.calls[0][0];
            expect(mailOptions.subject).toContain(`${emoji} Your Results: Grading Test (${percentage}%)`);
    ...
    describe("Private Helper Methods", () => {
        describe("escapeHtml", () => {
            test("escapes all HTML special characters: &, <, >, \", '", () => {
                const escapeHtml = emailService.escapeHtml.bind(emailService);
        ...
        describe("convertTitleToString", () => {
            test("returns string as is when input is a string", () => {
                const convertTitleToString = emailService.convertTitleToString.bind(emailService);
        ...
        describe("formatAnswer", () => {
            test("formats null and undefined as <em>No answer provided</em>", () => {
                const formatAnswer = emailService.formatAnswer.bind(emailService);
            ...
            test("formats empty array as <em>No selections made</em> and non-empty array with joined commas", () => {
                ...
                expect(formatAnswer(["Apple", "Banana", "Orange"])).toBe("Apple, Banana, Orange");
            });
