...
class AnalyticsController {
    GetAnalyticsData = async (req, res) => {
        ...
        try {
            ...
            const form = await ResponseValidationService_1.ResponseValidationService.validateFormAccess(formId, user.sub, res);
            if (!form) {
                return;
            }
    ...
    ProcessQuestionAnalytics = async (question, responses) => {
        if (!question._id)
            return;
    ...
    ProcessChoiceQuestion = (question, questionResponses, baseData) => {
        const choices = question?.[question.type];
    ...
    ProcessRangeQuestion = (question, questionResponses, baseData) => {
        const values = [];
        const scatterData = [];
        questionResponses.forEach((response) => {
            if (!response?.response)
    ...
    ProcessTextQuestion = (questionResponses, baseData) => {
    ...
    ProcessNumberQuestion = (questionResponses, baseData) => {
    ...
    ProcessDateQuestion = (questionResponses, baseData) => {
     ...
     * Calculate overall form statistics
     */
    CalculateFormStats = (responses, form) => {
