...
async function getAvailableColumns(req, res) {
    try {
        const { formId } = req.params;
        const form = await Form_model_1.default.findById(formId).populate("contents");
        if (!form) {
...
// Delete export job
async function deleteExportJob(req, res) {
    try {
        if (!req.user) {
            res.status(401).json((0, helper_1.ReturnCode)(401, "Authentication required"));
            return;
...
const processExportJob = async (jobId, formId, config) => {
    try {
        // Get form responses
        const responses = await Response_model_1.default.find({ formId }).populate("userId", "email");
...
// Generate export file
const generateExportFile = async (config, responses) => {
        ...
        columns.forEach((column) => {
            switch (column) {
                ...
                default:
                    ...
                    const fieldData = response.responseset?.find((r) => r.questionId.toString() === column);
