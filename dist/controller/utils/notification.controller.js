...
class SSEConnectionManager {
    ...
    addClient(userId, res) {
        ...
        console.log(`[SSE] Client added for user ${userId}. Total: ${this.clients.get(userId)?.length}`);
    }
...
class NotificationController {
    ...
    static async CreateNotification(data) {
        try {
            const notification = await Notification_model_1.default.create({
                ...
                formId: data.formId ? new mongoose_1.Types.ObjectId(data.formId) : undefined,
    ...
    GetNotificationSettings = async (req, res) => {
        ...
        try {
            ...
            res.status(200).json({ ...(0, helper_1.ReturnCode)(200), data: settings });
        }
        catch (error) {
    ...
    static async getAllFormRecipients(form) {
        const recipients = [];
    ...
    static async NotifyNewResponse(formId, responseId, respondentData) {
        try {
            ...
            const recipients = await NotificationController.getAllFormRecipients(form);
            // Create notifications for all recipients
            const notifications = await Promise.all(recipients.map((recipientId) => NotificationController.CreateNotification({
                userId: recipientId,
                type: "response",
    ...
    static async NotifyFormMilestone(formId, milestone) {
        try {
            const Form = require("../../model/Form.model").default;
            const form = await Form.findById(formId).populate("user");
            if (!form)
    ...
    static async NotifyFormReminder(formId, reminderType) {
        try {
            const Form = require("../../model/Form.model").default;
            const form = await Form.findById(formId).populate("user");
            if (!form)
            ...
            if (reminderType === "incompleteForm") {
                const existingReminder = await Notification_model_1.default.findOne({
