...
class FormResponseUtilityController {
    ...
    SendFormLinks = async (req, res) => {
        try {
            ...
            const userDetails = await User_model_1.default.findById(new mongoose_1.Types.ObjectId(validation.user.sub));
    ...
    GenerateFormLink = async (req, res) => {
        ...
        try {
            ...
            const form = await Form_model_1.default.findById(formId).select("_id user owners editors");
            if (!form) {
                return res.status(404).json((0, helper_1.ReturnCode)(404, "Form not found"));
            }
            ...
            return res.status(200).json({ ...(0, helper_1.ReturnCode)(200), data: link });
        }
        catch (error) {
            const err = error;
            console.error("Generate Form Link Error:", error);
            return res.status(500).json({
                ...(0, helper_1.ReturnCode)(500, "Failed to generate form link"),
                error: err?.message,
            });
        }
    };
    SendResponseCardEmail = async (req, res) => {
        try {
            ...
            const response = await Response_model_1.default.findById(responseId)
                .populate("formId")
                .populate({
            ...
            for (const responseItem of response.responseset) {
                ...
                if (questionType === Content_model_1.QuestionType.MultipleChoice && question.multiple) {
                    choices = question.multiple.map((opt) => ({
                        content: opt.content,
                        idx: opt.idx,
                        isCorrect: hasAnswerKey && Array.isArray(question.answer?.answer)
            ...
            if (success) {
                return res.status(200).json({
                    ...(0, helper_1.ReturnCode)(200, "Response card email sent successfully"),
                    data: {
    ...
    ExportResponsePDF = async (req, res) => {
        try {
            ...
            if (!form)
                return;
            const response = await Response_model_1.default.findOne({
    ...
    async generateResponsePDF(form, response) {
