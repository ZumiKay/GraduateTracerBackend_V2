...
const renderResponseValue = (response, question) => {
    ...
    if (options?.length > 0 && (question.checkbox || question.multiple || question.selection)) {
        return renderChoiceOptions(options, response);
    }
    return (0, helper_1.convertResponseToString)(response);
};
const buildQuestionsHtml = (processedResponseSet, form) => `
    ...
    return `
          <div class="question-item" style="background-color: #f9fafb; border: 1px solid #e5e7eb; border-radius: 8px; padding: 20px; margin-bottom: 15px;">
            ...
            <div style="margin: 12px 0;">
              <strong style="color: #374151; font-size: 14px;">${question.title}</strong>
            </div>
            ${isTextType ? "" : `
            ...
            ${!isTextType && question.hasAnswer && question.answer && form.type === Form_model_1.TypeForm.Quiz ? `
              <div style="background-color: #d1fae5; border-left: 3px solid #10b981; padding: 12px; margin-top: 10px; border-radius: 4px;">
                ...
                <div style="color: #065f46; font-size: 14px;">${renderResponseValue(typeof question.answer === "object" && "answer" in question.answer
...
const buildEmailHtml = (params) => {
    const { formTitle, totalScore, responseId, html, reason, feedback, questionsHtml } = params;
    return `
    <!DOCTYPE html>
    <html>
    <head>
            ...
            <table width="600" cellpadding="0" cellspacing="0" style="max-width: 600px; width: 100%; background-color: #ffffff; border-radius: 8px;">
              ...
              <tr>
                <td style="background-color: #f9fafb; padding: 30px 20px;">
                  ...
                  <table width="100%" cellpadding="20" cellspacing="0" style="background-color: white; border: 2px solid #f59e0b; border-radius: 8px; margin: 20px 0;">
                    <tr>
                      <td style="text-align: center;">
                        <div style="font-size: 36px; font-weight: bold; color: #f59e0b; margin-bottom: 10px;">${totalScore}</div>
                  ...
                  <p style="color: #6b7280; font-size: 14px;">Response ID: ${responseId}</p>
...
class FormResponseReturnController extends form_response_controller_1.FormResponseController {
    ReturnResponse = async (req, res) => {
        ...
        const responseIds = Array.isArray(responseId) ? responseId : [responseId];
        ...
        try {
            const formResponses = await Response_model_1.default.find({ _id: { $in: responseIds } })
                .select("responseset formId totalScore respondentEmail")
                .lean();
            if (!formResponses.length) {
                return res.status(404).json({ message: "Response not found" });
            }
            const formId = formResponses[0].formId;
            ...
            const contents = await Content_model_1.default.find({ formId })
            ...
            const numberedContent = (0, helper_1.AddQuestionNumbering)({ questions: contents });
            await Promise.all(formResponses.map(async (response) => {
                if (!response.respondentEmail)
                    return;
                let questionsHtml = "";
                if (includeQuestionsAndResponses) {
                    const processedResponseSet = ResponseQueryService_1.ResponseQueryService.ResponsesetProcessQuestion(numberedContent, response.responseset, { filterHidden: true });
                    questionsHtml = buildQuestionsHtml(processedResponseSet, form);
                }
