...
const ManageFormCollaborator = async (req, res) => {
    try {
        ...
        const targetUser = await User_model_1.default.findOne({ email }).exec();
        if (!targetUser)
        ...
        if (currentField) {
            if (action === "add") {
                ...
                const existingPendingInvite = form.pendingCollarborators?.find((pc) => pc.user.toString() === collaboratorId.toString());
                ...
                if (existingPendingInvite) {
                    ...
                    await Form_model_1.default.findOneAndUpdate({ _id: formId, "pendingCollarborators.user": collaboratorId }, {
                        $set: {
                    ...
                    await Form_model_1.default.findByIdAndUpdate(formId, {
                        $addToSet: {
                ...
                const inviter = await User_model_1.default.findById(user.sub).lean();
...
async function ConfirmAddCollaborator(req, res) {
    ...
    try {
        ...
        const pendingCollaborator = form.pendingCollarborators?.find((pc) => pc.code === inviteCode);
        ...
        return res.status(200).json({
            ...(0, helper_1.ReturnCode)(200, `You have been added as ${role} to the form`),
            data: {
                formId,
                formTitle: form.title,
                role,
...
async function GetFormCollaborators(req, res) {
    ...
    try {
        ...
        const pendingCollaborators = form.pendingCollarborators?.map((pending) => {
        ...
        return res.status(200).json({
            ...(0, helper_1.ReturnCode)(200, "Form collaborators retrieved successfully"),
            data: {
                primaryOwner,
                allOwners,
                allEditors,
                ...
                pendingOwnershipTransfer,
                totalCollaborators: (allOwners?.length ?? 0) + (allEditors?.length ?? 0),
            },
        });
    }
    catch (error) {
...
async function RemoveSelfFromForm(req, res) {
    const { formId } = req.params;
    const currentUser = req.user;
...
async function ChangePrimaryOwner(req, res) {
    ...
    try {
        const form = await Form_model_1.default.findById(formId).populate("user", "email").lean();
        if (!form) {
            return res.status(404).json((0, helper_1.ReturnCode)(404, "Form not found"));
        }
        if (!(0, formHelpers_1.isPrimaryOwner)(form, currentUser.sub)) {
        ...
        // Get the target user
        const targetUser = await User_model_1.default.findById(userId).lean();
        if (!targetUser) {
            return res.status(404).json((0, helper_1.ReturnCode)(404, "User not found"));
        }
        ...
        const currentOwner = await User_model_1.default.findById(currentUser.sub).lean();
...
async function ConfirmOwnershipTransfer(req, res) {
    ...
    try {
        ...
        await Form_model_1.default.findByIdAndUpdate(formId, updateOps);
        ...
        if (Object.keys(pullOps).length > 0) {
            await Form_model_1.default.findByIdAndUpdate(formId, { $pull: pullOps });
        }
        return res.status(200).json({
            ...(0, helper_1.ReturnCode)(200, "Ownership transfer completed successfully. You are now the primary owner of this form."),
            data: {
                formId,
                formTitle: form.title,
                role: "CREATOR",
...
async function ResendPendingInvitation(req, res) {
    ...
    try {
        ...
        const pendingCollaborator = form.pendingCollarborators?.find((pc) => pc._id.toString() === pendingId);
        if (!pendingCollaborator) {
            return res
                .status(404)
        ...
        while (!isUnique) {
            const isCode = form.pendingCollarborators?.some((c) => c.code === newInviteCode && c._id.toString() !== pendingId);
            if (!isCode)
                isUnique = true;
            else
        ...
        await Form_model_1.default.updateOne({ _id: formId, "pendingCollarborators._id": pendingId }, {
            $set: {
        ...
        // Get inviter's email
        const inviter = await User_model_1.default.findById(currentUser.sub).lean();
        const inviterEmail = inviter?.email || "A form owner";
...
async function DeletePendingCollaborator(req, res) {
    const { formId, pendingId } = req.body;
    const currentUser = req.user;
    if (!currentUser) {
        return res.status(401).json((0, helper_1.ReturnCode)(401, "Unauthorized"));
